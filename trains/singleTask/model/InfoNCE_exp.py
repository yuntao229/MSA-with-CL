...
def info_nce_mul(batch, labels, temperature=0.1, reduction='mean', negative_mode='unpaired'):
    ...
    positive_key, negative_keys, negative_weights = choose_samples(batch, labels)
    ...
    negative_keys = torch.tensor([item.cpu().detach().numpy() for item in negative_keys], device=batch.device)
    negative_weights = torch.tensor(negative_weights, device=batch.device)
    query, positive_key, negative_keys = normalize(batch, positive_key, negative_keys)
    ...
    logits = torch.cat([positive_logit, negative_logits], dim=1)
    labels = torch.zeros(len(logits), dtype=torch.long, device=query.device)
    print("logits_mul:", logits, "labels_mul:", labels)
    
    return weighted_entropy(logits / temperature, labels, negative_weights)
...
def choose_samples(batch_data, labels):
        ...
        for iter in range(labels.shape[-1]):
            if iter != i:
                ...
                if labels[iter] * labels[i] < 0: ### Choose opposite emotion samples as negative keys while calculating weighted parameters.
                    ...
                    beta = 1 - 1 / (1 + math.exp(-1 * delta))
                    negative_keys_list.append(batch_data[iter]) ### Add negative sample into negative keys(2 dimensions).
                    negative_keys_weights.append(beta) ### Save weights of negative samples, corresponded by index.
            negative_keys_ret.append(negative_keys_list)
            negative_weights_ret.append(negative_keys_weights)
    ...
    assert len(negative_keys_ret) == len(negative_weights_ret), "Length of negative samples is not corresponding to negative weights!"
    assert len(negative_keys_ret) == len(positive_key_list), "Length of negative groups is not corresponding to positive samples!"
...
def choose_samples_similarity(batch_data, labels):
    ...
    for i in range(batch_data.shape[0]):
        ...
        for iter in range(labels.shape[-1]):
            if iter != i:
                similarity = torch.cosine_similarity(batch_size[i], batch_size[iter])
...
if __name__ == "__main__":
    loss = InfoNCE()
    batch_size, num_negative, embedding_size = 2, 4, 8
    ...
    output = loss(query, positive_key, negative_keys)
    print("output:", output)
    loss_multi = InfoNCE_mul()
    print("mult-label output:", loss_multi(query, labels))
